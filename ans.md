...
`printf() -> console_putc() -> uartputc() -> 硬件寄存器`
1. 每一层的职责是什么？
- printf()：​格式化解析​（处理%d等格式，数字转字符串）
- console_putc()：​控制台抽象​（统一字符输出接口，可能处理缓冲/转义）
...
- ​可扩展性​：中间层（如console_putc()）可插入缓冲/日志等功能
...
- 时钟中断为什么在M模式产生，却在S模式处理？
  - M 模式负责接收硬件信号并转发，S 模式负责执行具体的中断处理代码。
  - 操作系统内核运行在 S 模式（监管者模式）。内核的进程调度算法需要利用时钟中断来切片时间，所以逻辑上必须由 S 模式来处理。
- 如何理解中断是异步的，异常是同步的。
  - 中断是异步的 (Asynchronous)：来源外部： 来自 CPU 外部的设备（如时钟、网卡、键盘）。关系： 与当前执行的指令无关。 它可以在 CPU 执行任何一条指令的空隙突然发生。特点： 不可预测，CPU 是被动接收。
  - 异常是同步的 (Synchronous)：来源内部： 来自 CPU 内部执行单元。关系： 由执行当前指令直接触发。 例如：执行了 ecall（系统调用）、读了空指针（缺页）、或者执行了乱码（非法指令）。特点： 可预测（可复现）。只要指令流和环境不变，异常一定会在代码的同一个位置重复发生。

### 分析xv-6的中断处理流程

1. 研读`start.c`中的机器模式设置
   - 为什么时钟中断需要特殊处理
     - RISC-V 架构规定，时钟硬件（CLINT）是 M 模式（最高特权级） 的资源。而你的内核运行在 S 模式。S 模式不能直接复位硬件时钟比较器。原始 XV6 为了教学写了 timervec 在 M 模式转发中断；你的代码利用 SBI 委托机制，简化了这一步，直接在 S 模式接收中断。SBI 调用 (ecall)，相当于请求 OpenSBI（运行在 M 模式）帮你去操作底层硬件。
   - `timevec`的作用是什么？
     - 对应物是 kernelvec。因为 OpenSBI 已经帮你把 M 模式的时钟中断 委托（Delegation） 给了 S 模式，所以时钟中断发生时，CPU 直接跳转到你的 kernelvec，而不是 M 模式的向量表。
2. 分析`kernelvec.S`的上下文切换
   - 哪些寄存器需要保存 保存了 所有通用寄存器
   - 为什么不保存所有寄存器
     - 内核代码（以及中断处理函数）没有使用浮点运算。保存浮点寄存器非常耗时（数据量大），为了性能，除非明确使用了浮点单元，否则通常不保存。
   - 栈的使用策略
3. 理解`trap.c`的中断分发：
   - 中断处理中的重入问题如何解决？
   - 中断处理时间过长会有什么后果？
